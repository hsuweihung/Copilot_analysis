...
df = pd.read_excel("C:\\Users\\William_Hsu\\Desktop\\PDR_Copilot\\Copilot_unhelp.xlsx")

df = df.dropna(subset=['Prompt', 'Copilot Response'])
...
def process_topic_modeling(X, version_type, original_length):
    ...
    clusterer = hdbscan.HDBSCAN(min_cluster_size=5, min_samples=2, cluster_selection_method='eom')
    labels = clusterer.fit_predict(X_umap)
    
    unique_labels = set(labels)
    print(f"{version_type} 群集標籤: {unique_labels}")

    optimal_k = max(2, min(len(unique_labels) - (1 if -1 in labels else 0), 10))
    ...
    lda = LatentDirichletAllocation(n_components=optimal_k, learning_method='online', random_state=42)
...
topics_essential, lda_essential, _ = process_topic_modeling(X_essential, "Essential", len(essential_prompts))
df.loc[df['Versiontype'] == 'Essential', 'Essential_Topic'] = topics_essential.argmax(axis=1)

topics_subscription, lda_subscription, _ = process_topic_modeling(X_subscription, "Subscription", len(subscription_prompts))
...
def print_keywords(lda_model, version_type, n_words=10):
    ...
    for topic_idx, topic in enumerate(lda_model.components_):
        print(f"Topic #{topic_idx}: " + " ".join([terms[i] for i in topic.argsort()[:-n_words - 1:-1]]))
...
tfidf_matrix = tfidf_vectorizer.fit_transform(df['Prompt'].tolist() + df['Copilot Response'].tolist())
tfidf_prompt = tfidf_matrix[:len(df)]
tfidf_response = tfidf_matrix[len(df):]
df['Cosine_Similarity'] = [cosine_similarity(tfidf_prompt[i], tfidf_response[i])[0][0] for i in range(len(df))]

df_high_sim = df[df['Cosine_Similarity'] > 0.8]
print("\n=== 高相似度但仍 Unhelp 的案例 ===")
print(df_high_sim[['Prompt', 'Copilot Response', 'Cosine_Similarity']])

df['Response_Length'] = df['Copilot Response'].apply(lambda x: len(x.split()))
df['Response_Length_Group'] = pd.cut(df['Response_Length'], bins=[0, 10, 30, 60, 100, np.inf], labels=['<10', '10-30', '30-60', '60-100', '100+'])

print("\n=== 不同回應長度的分布 ===")
print(df['Response_Length_Group'].value_counts())

negative_keywords = ['sorry', 'no information', 'not sure', 'unavailable', 'i don’t know']
df['Has_Negative_Words'] = df['Copilot Response'].apply(lambda x: any(word in x.lower() for word in negative_keywords))
