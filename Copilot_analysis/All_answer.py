...
df = pd.read_excel(
    "/Users/williamhsu/Desktop/Copilot_analysis/Copilot_analysis/User_all_qusetion.xlsx")

essential_prompts = df[df['Versiontype'] ==
                       'Essential']['Prompt'].dropna().values
subscription_prompts = df[df['Versiontype'] ==
                          'Subscription']['Prompt'].dropna().values
...
print("Checkpoint 1: Before running UMAP")
...
optimal_k_essential = len(set(labels_essential)) - \
    (1 if -1 in labels_essential else 0)
optimal_k_essential = min(optimal_k_essential, 10)
print(f"Essential最佳主題數量（n_components）: {optimal_k_essential}")

lda_essential = LatentDirichletAllocation(
    n_components=optimal_k_essential, learning_method='online', n_jobs=-1, random_state=42)
...
reducer_subscription = umap.UMAP(
    n_neighbors=15, n_components=10, random_state=42)
X_umap_subscription = reducer_subscription.fit_transform(
    X_subscription.toarray())
...
optimal_k_subscription = len(
    set(labels_subscription)) - (1 if -1 in labels_subscription else 0)
optimal_k_subscription = min(optimal_k_subscription, 10)
print(f"Subscription最佳主題數量（n_components）: {optimal_k_subscription}")

lda_subscription = LatentDirichletAllocation(
    n_components=optimal_k_subscription, learning_method='online', n_jobs=-1, random_state=42)
